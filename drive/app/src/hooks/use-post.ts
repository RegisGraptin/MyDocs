...
export function usePost() {
  ...
  const [contextId, setContextId] = React.useState<string>('');
   ...
   useEffect(() => {
    ...
    const initializeApi = async () => {
      try {
        console.log('[usePost] creating API client');
        console.log(app);
        ...
        try {
          ...
          if (documentData) {
            console.log('[usePost] fetched document data during init', documentData);
          }
        } catch (e) {
  ...
  const { doc, isSubscribed, error, subscribe, unsubscribe, refresh } = useDocument({
    contextId,
    onUpdate: (d) => {
      ...
      if (d && typeof d.content === 'string') {
        try {
          ...
          console.warn('[usePost] failed to parse on-chain document content', e);
        }
      }
    },
  });
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (app) {
        try {
          const saved = typeof window !== 'undefined' ? window.localStorage.getItem('calimero_contextId') : null;
          const contexts = await app.fetchContexts();
          let hasContext = false;
          if (saved) hasContext = contexts.some((c: any) => c.contextId === saved);
          if (!hasContext && contexts && contexts.length > 0) hasContext = true;

          if (hasContext) {
            // ensure contextId is set so other hooks can subscribe
            let selected = null as any;
            if (saved) selected = contexts.find((c: any) => c.contextId === saved) || null;
            if (!selected && contexts && contexts.length > 0) selected = contexts[0];
            if (selected) {
              setContextId(selected.contextId);
              try { if (typeof window !== 'undefined') window.localStorage.setItem('calimero_contextId', selected.contextId); } catch {}
            }
            ...
            try {
              ...
              console.warn('[usePost] failed to load post from chain, falling back', e);
            }
          } else {
            console.log('[usePost] no Calimero contexts found, skipping on-chain load');
          }
        } catch (e) {
          console.warn('[usePost] error checking contexts, falling back to local', e);
        }
      }
  ...
  return { savePost, debouncedSave, saveStatus, isLoading, post, isSubscribed, error, contextId };
}
